...
import { useTrailContext } from "./context/TrailContextProvider";
...
const TrailElevationGraph: React.FC<TrailElevationGraphProps> = ({
  geoJsonData /* , onHover */,
}) => {
  // const [hoverData, setHoverData] = useState<{ elevation: number; distance: number } | null>(null);
  const { chartContainerRef, svgRef } = useTrailContext();
  ...
  useEffect(() => {
    ...
    const distances = geoJsonData.features[0].geometry.coordinates.map(
      (c: any) => c[0],
    );
    ...
    const yScale = scaleLinear()
      .domain([Math.min(...elevations), Math.max(...elevations)])
